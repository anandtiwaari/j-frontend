...
const Dashboard = () => {
    ...
    const getAllTheEmails = async () => {
        try {
            const response = await axios.get('https://j-backend-69vt.onrender.com/api/scheduled-emails');
        ...
        return (
            <List.Item key={email._id}>
                ...
                >
                            ...
                            <>
                                {showAllEmails[email._id] ?
                                    ...
                                    <>
                                        ...
                                        <Button
                                            type="link"
                                            onClick={() => setShowAllEmails(prev => ({ ...prev, [email._id]: true }))}
                                        >
                                            More
                                        </Button>
                                    </>
                                ...
                                {showAllEmails[email._id] &&
                                    <Button
                                        type="link"
                                        onClick={() => setShowAllEmails(prev => ({ ...prev, [email._id]: false }))}
                                    >
                                        Less
                                    </Button>
                                }
                    ...
                    <p><strong>Scheduled At:</strong> {new Date(email.scheduledAt).toLocaleString()}</p>
                    {/* <p><strong>Status:</strong> {email.status}</p>
                     */}
                    <p>
                        <strong>Status:</strong>
                        <Tag className="ml-2 font-bold" color={email.status === 'pending' ? 'volcano' : 'success'}
                         >
                            {email.status}
                        </Tag>
                    </p>
                    <p><strong>Created At:</strong> {new Date(email.createdAt).toLocaleString()}</p>
                    <p><strong>Updated At:</strong> {new Date(email.updatedAt).toLocaleString()}</p>
                </Card>
            </List.Item>
        );
    };
